...
import '../base/base_crop_photo.dart';
import '../base/base_sex.dart';
import '../friends/friends_friend_status_status.dart';
import '../friends/friends_requests_mutual.dart';
import '../owner/owner_state.dart';
import '../users/users_career.dart';
import '../users/users_exports.dart';
import '../users/users_last_seen.dart';
import '../users/users_military.dart';
import '../users/users_occupation.dart';
import '../users/users_online_info.dart';
import '../users/users_personal.dart';
import '../users/users_relative.dart';
import '../users/users_school.dart';
import '../users/users_university.dart';
import '../users/users_user_min.dart';
import '../users/users_user_relation.dart';
import 'groups_role_options.dart';

class VKGroupsUserXtrRole {
  ...
  final VKGroupsRoleOptions role;

  const VKGroupsUserXtrRole({
    this.deactivated,
    ...
    this.role,
  })  : assert(followersCount >= 0),
        assert(videoLiveLevel >= 0),
        assert(videoLiveCount >= 0),
  ...
  factory VKGroupsUserXtrRole.fromMap(Map<String, dynamic> map) {
    return VKGroupsUserXtrRole(
      ...
      career: map['career']?.map((item) => VKUsersCareer.fromMap(item)),
      military: map['military']?.map((item) => VKUsersMilitary.fromMap(item)),
      ...
      universities:
          map['universities']?.map((item) => VKUsersUniversity.fromMap(item)),
      schools: map['schools']?.map((item) => VKUsersSchool.fromMap(item)),
      relatives: map['relatives']?.map((item) => VKUsersRelative.fromMap(item)),
      ...
      role: VKGroupsRoleOptions(map['role']),
    );
  }
}
