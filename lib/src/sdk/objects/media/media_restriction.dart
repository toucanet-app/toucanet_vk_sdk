...
import '../video/video_restriction_button.dart';
...
class VKMediaRestriction {
  ...
  factory VKMediaRestriction.fromMap(Map<String, dynamic> map) {
    return VKMediaRestriction(
      ...
      cardIcon: map['card_icon']?.map((item) => VKBaseImage.fromMap(item)),
      listIcon: map['list_icon']?.map((item) => VKBaseImage.fromMap(item)),
    );
  }
}
